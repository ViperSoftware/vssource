...
//
// See the sample TOOLBAR in the \OT\SAMPLES\STANDARD\DOCKING\TOOLBAR directory for a demonstration of this class.
...
public:
	...
	int IDToBmpIndex(UINT nID, HBITMAP* lphBmp);
	...
	void SetToolBarInfo(int nImgWidth, int nImgHeight);
...
public:
	...
	inline int GetImgWidth() const { return m_pBmpMgr->GetImgWidth(); }
	inline int GetImgHeight() const { return m_pBmpMgr->GetImgHeight(); }
	inline int GetStdBtnWidth() const { return (GetImgWidth() + 7); }
	inline int GetStdBtnHeight() const { return (GetImgHeight() + 7); }
	inline int GetSepWidth() const { return ((GetImgWidth() + 8) / 3); }
	inline int GetSepHeight() const { return (GetImgHeight() + 7); }

	// Modifies m_bLocked
	inline void LockToolBar(BOOL bLock) { m_bLocked = bLock; }
