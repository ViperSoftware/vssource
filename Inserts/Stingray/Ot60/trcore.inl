...
{
	InsertColumn( 0, _T(""), LVCFMT_CENTER, 0, 0, FALSE );
...
BOOL SEC_TREECLASS::SubclassTreeCtrlId( UINT idc, CWnd* pWndDlg )
{
	...
	//We want tooltips on by default to be plug compatible with MS SysTreeView32
	BOOL bOk = Create( dwStyle, dwExStyle|TVXS_FLYBYTOOLTIPS, rect, pWndDlg, idc );
...
//		TVIR_BOUNDS	Returns the bounding rectangle of the entire item, including the icon and label.
//		TVIR_ICON	Returns the bounding rectangle of the icon or small icon.
//		TVIR_LABEL	Returns the bounding rectangle of the item text.
...
HTREEITEM SEC_TREECLASS::GetPrevItemInDisplayOrder(HTREEITEM hti) const
{
	Node *pNode=GetNode(hti);
	ASSERT(pNode);
    return (HTREEITEM) pNode->GetPrevInDisplayOrder();
}
	...
	if (pLVI->iSubItem == 0)
	{
		...
		if (tvi.mask & TVIF_STATE)
		{
			...
			if (tvi.stateMask & TVIS_STATEIMAGEMASK)
				pLVI->state |= (tvi.state & TVIS_STATEIMAGEMASK) ? LVIS_STATEIMAGEMASK : 0;
		}
...
BOOL SEC_TREECLASS::SetItem(TV_ITEM* pTVI)
{
	...
	if (iItem != -1)
	{
		...
		if (pTVI->mask & TVIF_STATE)
		{
			...
			{
				lvi.state |= (pTVI->state & TVIS_STATEIMAGEMASK) ? LVIS_STATEIMAGEMASK : 0;
...
BOOL SEC_TREECLASS::SetItem(const LV_ITEM* pLVI, BOOL bRedraw /*=TRUE*/ )
{
	if (pLVI->iSubItem == 0)
	{
		...
		if (pLVI->mask & LVIF_STATE)
		{
			...
			if (pLVI->stateMask & LVIS_STATEIMAGEMASK)
			{
				tvi.state |= (pLVI->state & LVIS_STATEIMAGEMASK) ? TVIS_STATEIMAGEMASK : 0;
				tvi.stateMask |= TVIS_STATEIMAGEMASK;
...
SEC_TREECLASS::GetNode( HTREEITEM hti ) const
{
	if( !hti || hti==TVI_ROOT )
		return m_pNodeRoot;
							...
							HTREEITEM hInsertAfter)
{
	...
	else
	{
		...
		tvi.cchTextMax = _tcslen(lpszItem) + 1;
	}
...
//@parm  BOOL | bRedraw | If TRUE, the tree will be invalidated after expansion. Default is TRUE.
...
BOOL SEC_TREECLASS::Expand( HTREEITEM htiExpand, UINT nCode, BOOL bRedraw )
{
...
BOOL SEC_TREECLASS::SelectDropTarget(HTREEITEM htiDropHilightItem, BOOL bAutoScroll /* TRUE */)
{
	if (htiDropHilightItem != NULL)
	{
		...
		return SEC_TREEBASE::SelectDropTarget(nIndex);
	}
	else
...
CImageList* SEC_TREECLASS::CreateDragImage(HTREEITEM hti)
{
	...
	//ASSERT -> Can't create a drag image of multiple items right now...
...
UINT SEC_TREECLASS::GetChildCount(HTREEITEM hti, BOOL bRecursive)
{
	Node *pParent;
	Node *pNode = 0;
	UINT nResult = 0;
	...
	{
		nResult++;
		if (bRecursive && pNode->HasChildren())
			nResult += GetChildCount( (HTREEITEM)pNode, bRecursive );
...
BOOL SEC_TREECLASS::DisableItem( HTREEITEM hti, BOOL bDisable )
{
	...
	if( IsDisabled((HTREEITEM)pNodeDis))
	{
...
BOOL SEC_TREECLASS::SetTreeCtrlStyle(DWORD dwStyle, BOOL bRedraw)
{
	...
	m_dwTreeStyle = (dwStyle&(TVS_HASLINES|TVS_LINESATROOT|TVS_HASBUTTONS|TVS_DISABLEDRAGDROP));
...
void SEC_TREECLASS::EnableHeaderCtrl(BOOL bEnable)
{
	if ( bEnable )
	...
	SEC_TREEBASE::EnableHeaderCtrl( bEnable );
	...
	if (pPC->lvi.iSubItem == 0)
	{
		...
		if (mask & TVIF_STATE)
		{
			...
			if (stateMask & TVIS_STATEIMAGEMASK)
				pPC->lvi.state |= (pTvPC->tvi.state & TVIS_STATEIMAGEMASK) ? LVIS_STATEIMAGEMASK : 0;
		}
...
void SEC_TREECLASS::MeasureText(CDC* pDC, Node* pNode, const CString& strText, CRect& rectText) const
{
	...
	else
	{
		...
		DWORD dwSize = ::GetTabbedTextExtent( hdc, strText,	_tcslen(strText), 0, NULL );
...
void SEC_TREECLASS::MeasureSubItem(LvPaintContext* pPC, UINT& cySubItem)
{
	if (pPC->lvi.iSubItem == 0)
	{
		...
		cySubItem = otmax(pPC->pItem->rcIcon.Height(), pPC->pItem->rcStateIcon.Height()) ;
		cySubItem = otmax((UINT)pPC->pItem->rcText.Height(), cySubItem);
...
void SEC_TREECLASS::DrawSubItem(LvPaintContext* pPC)
{
	if (pPC->lvi.iSubItem == 0)
	{
		...
		if( pPC->da == DrawEntire && bHasImage)
		{
			...
			UINT nStyle = iOimage > 0 ? 
				INDEXTOOVERLAYMASK(iOimage) | ILD_NORMAL :
				ILD_NORMAL;
...
void SEC_TREECLASS::OnRightKeyPressed(int nItem, int nSubItem, BOOL bShift, BOOL bCtrl, UINT nRepCnt, UINT nFlags)
{
	...
	if (ItemHasChildren(htiSel) && IsExpanded(htiSel))
	{
		...
		DeselectAllItems();
		VERIFY(Select(htiChild, TVGN_CARET));
	}
	else if (ItemHasChildren(htiSel) || ItemHasChildrenOnDemand(htiSel))
		VERIFY(Expand(htiSel, TVE_EXPAND));
...
BOOL SEC_TREECLASS::AddNodeToListBox( Node* pNodeAdd )
{
	...
	pItem->SetItemData((DWORD)pNodeAdd);
...
void SEC_TREECLASS::RemoveChildrenFromListCtrl(Node* pParent, int nParent)
{
	...
	int val;
	for (i=m_arrSelItems.GetUpperBound(); i>=0; i--)
	{
...
void SEC_TREECLASS::OnLButtonDown( UINT nFlags, CPoint point )
{
	...
	if( iHitFlags&TVHT_ONITEMBUTTON )
	{
		Expand(htiHit, TVE_TOGGLE );
	}
	else
...
CBitmap* SEC_TREECLASS::BitmapFromText( CDC* pdc, const CString& rStr )
{
	...
	sizeText = pdc->GetTextExtent( rStr );
	...
	pdc->DrawText( rStr, rectText, DT_CENTER|DT_EXPANDTABS );
