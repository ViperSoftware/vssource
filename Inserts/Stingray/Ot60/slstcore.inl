...
void SEC_LISTCLASS::PickTextColors(LvPaintContext* pPC)
{
    ...
    if( (pPC->lvi.iItem == GetDropHilightIndex()) && (pPC->lvi.iSubItem == 0 ) )
    {
        COLORREF rgb;
        rgb = pPC->rgbText;
        pPC->rgbText = pPC->rgbTextBkgnd;
        pPC->rgbTextBkgnd = rgb;
    }
}
...
BOOL SEC_LISTCLASS::InsertColumn(int nCol, const CString& strHeader, int fmt, int widthCol, int iSubItem, BOOL bUpdate)
{
    //insert Item info
...
BOOL SEC_LISTCLASS::InsertColumn( int nCol, const LV_COLUMN* lvc, BOOL bUpdate )
{
	...
	return InsertColumn( nCol, strHeader, fmt, cx, iSubItem, bUpdate );
}
...
BOOL SEC_LISTCLASS::AddColumn( const CString& strHeader, int fmt, int widthCol, int iSubItem)

//
// Add a column to the whole SEC_LISTCLASS
//
{
    return  InsertColumn( GetColumnCount(), strHeader, fmt, widthCol, iSubItem );
}
...
{
	ModifyListCtrlStyleEx(!bEnable*LVXS_FLYBYTOOLTIPS, bEnable*LVXS_FLYBYTOOLTIPS);
}
...
void SEC_LISTCLASS::EnableHeaderCtrl(BOOL bEnable)
{
	ModifyListCtrlStyle(bEnable*LVS_NOCOLUMNHEADER, !bEnable*LVS_NOCOLUMNHEADER);
}
...
BOOL SEC_LISTCLASS::FindInSelItemsArray(int iItem, int& nIndex) const
{
	...
	lb = 0;
	nIndex = ub = m_arrSelItems.GetSize();
...
BOOL SEC_LISTCLASS::DeleteItem( int nItem, int cCount )
{
	...
	for (i=m_arrSelItems.GetUpperBound(); i>=nIndex; i--)
	{
...
void SEC_LISTCLASS::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if( nDragIndex != -1 && m_bLookForBeginDrag && (nFlags&MK_LBUTTON || nFlags&MK_RBUTTON) )
	{
...
void SEC_LISTCLASS::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if( (GetListCtrlStyle() & LVS_EDITLABELS)	&&
		...
		((GetListCtrlStyleEx() & LVXS_HILIGHTSUBITEMS) && (iHitFlags & LVHT_INSIDEITEMBOUNDS) && (m_nActiveColumn > 0) ) )
	 )
	{
...
void SEC_LISTCLASS::OnHomeKeyPressed(int nItem, int nSubItem, BOOL bShift, BOOL bCtrl, UINT nRepCnt, UINT nFlags)
{
	...
	if ( GetItemCount() > 0 )
	{
		...
		if ( bVScroll )
		{
			...
			if ( pBar )
			{
				SendMessage( WM_VSCROLL, MAKELPARAM( SB_TOP, 0), (long)pBar->m_hWnd );
				Invalidate();
			}
		}
...
void SEC_LISTCLASS::OnPgUpKeyPressed(int nItem, int nSubItem, BOOL bShift, BOOL bCtrl, UINT nRepCnt, UINT nFlags)
{
	...
	else
	{
		...
		if ( pBar && (nItem == GetTopIndex()) )
			SendMessage( WM_VSCROLL, MAKELPARAM( SB_PAGEUP, 0), (long)pBar->m_hWnd );
...
void SEC_LISTCLASS::OnPgDnKeyPressed(int nItem, int nSubItem, BOOL bShift, BOOL bCtrl, UINT nRepCnt, UINT nFlags)
{
	...
	else
	{
		...
		if ( pBar && (nItem == GetLastFullyVisibleItem()) )
			SendMessage( WM_VSCROLL, MAKELPARAM( SB_PAGEDOWN, 0), (long)pBar->m_hWnd );
...
void SEC_LISTCLASS::OnEndKeyPressed(int nItem, int nSubItem, BOOL bShift, BOOL bCtrl, UINT nRepCnt, UINT nFlags)
{
	...
	if (GetItemCount() > 0)
	{
		...
		if ( bVScroll )
		{
			...
			if ( pBar )
			{
				SendMessage( WM_VSCROLL, MAKELPARAM( SB_BOTTOM, 0), (long)pBar->m_hWnd );
				Invalidate();
			}
		}
...
void SEC_LISTCLASS::OnKeyDown( UINT nChar, UINT nRepCnt, UINT nFlags )
{
	...
	case VK_MULTIPLY:
		...
		break;
	case VK_SPACE:
		SelectFocusItem(nItem, !IsSelected(nItem));
		break;
	case VK_TAB:
		pWnd = GetParent();
		if ( pWnd )
	...
	else if ( ! (GetListCtrlStyle() & LVS_SINGLESEL) )
	{
		if( ! m_bAbortPendingSelect )
		{
			if (nFlags & MK_CONTROL)
			{
				...
				else // set the focus, toggle the selection
				{
					m_pToolTipWnd->Hide();
					m_bAbortPendingSelect = NotifySelChanging(nHitIndex, nPrevSel,sa);
					if(m_bAbortPendingSelect)
	...
	) const
{
	...
	if( m_bDragging )//|| IsHeaderCtrlEnabled() )
	{
		pMemDC = pScreenDC;
		return FALSE;
	}
	...
	try
	{
		pMemDC = new SECMemDC( pScreenDC, rectBitmap );
	}
