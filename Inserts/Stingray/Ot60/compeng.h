 ...
 * save space in the various tables. ISecPos is used only for parameter passing.
 */
typedef struct deflate_internal_state {
    ...
    ISecPos prev_match;             /* previous match */
...
public:
	...
	int longest_match(deflate_state *s, ISecPos cur_match);
  ...
  WORD inflate_hufts;
	...
	void check_match(deflate_state *s, ISecPos start, ISecPos match, int length);
#endif   
