...
//
// See the TABDEMO sample in the \OT\SAMPLES\STANDARD\TABS\TABDEMO directory for a demonstration of SEC3DTabControl.
...
protected:
	void SizeTabs(int nTot, int nTIcon, int nDim);
	void SwapStrings(CString& strX, CString& strY);
	void AddEllipsis();
	void RemEllipsis();
	int GetEllipseSize();
